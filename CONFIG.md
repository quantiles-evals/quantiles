...
Only one may exist in a given directory. The search is rooted at the current working directory where you invoke `qt run` or `qt resume`.
...
|-------|------|----------|-------------|
| `type` | string | no | Always `"builtin"`. May be omitted. |
...
Custom evaluations are external programs run as child processes. Their config sections **must** contain:
...
Supported provider prefixes: `openai:`, `anthropic:`, `gemini:`, `cloudflare:`, `random`, `random_label`.
...
For `custom_code` benchmarks, `input` is an arbitrary TOML table that becomes a JSON object:
...
- You do **not** need to re-pass `--input` on resume.
- If you edited the config file between `qt run` and `qt resume`, the resumed run uses the **updated** command.
