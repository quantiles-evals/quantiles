...
def _extract_context(row: RawPubmedQARowModel) -> str:
  ...
  if isinstance(contexts, list):
    return "\n".join(part for part in (_coerce_text(item) for item in contexts) if part)
  if isinstance(contexts, dict):
    ordered_keys = ["label", "contexts", "context", "abstract", "title"]
    values = [_coerce_text(contexts.get(key)) for key in ordered_keys]
    values.extend(_coerce_text(value) for key, value in contexts.items() if key not in ordered_keys)
...
def _transform_pubmedqa_row(raw: dict[str, JsonValue]) -> PubmedQARow:
  ...
  if sample_id == "":
    sample_id = hashlib.sha256(f"{question}|{context}|{gold_answer}".encode()).hexdigest()[:16]
...
def _build_messages(question: str, context: str) -> list[LLMMessage]:
  system_prompt = (
    "You are grading a biomedical QA item. Reply with exactly one label: yes, no, or maybe."
  )
  user_prompt = (
    f"Question:\n{question}\n\n"
...
async def _pubmedqa_handler(
  ...
  if ":" not in str(model_name):
    raise ValueError(f"model_name must be in 'provider:model_id' format, got: {model_name}")
