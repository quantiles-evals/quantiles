# T3 Airline `custom_code` example
...
Install `uv`, make Python 3.12 available to it, and run the following one-time setup from the `custom-code-examples` directory:

```bash
git clone --branch v1.0.1 --depth 1 https://github.com/sierra-research/tau2-bench.git t3-airline/.tau3/tau2-bench
```
...
```bash
qt run t3-airline --input '{"agent_model":"openai:gpt-4.1","user_model":"openai:gpt-4.1","task_ids":["0"],"num_trials":1}'
```
