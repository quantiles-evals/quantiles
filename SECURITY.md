...
- Local Quantiles workspace storage, including `.quantiles/quantiles.sqlite` and `.quantiles/metrics/`
- Dataset loading, caching, and export behavior that could expose private data or execute untrusted content
- Hosted Quantiles web services, dashboard, documentation site, and API routes
- Authentication, authorization, session handling, and Firebase/GCP integration
- Cloud infrastructure, deployment configuration, GitHub Actions, release artifacts, and package publishing
- Secret handling for provider API keys, model credentials, service accounts, and cloud storage access
- Vulnerabilities that could expose customer data, private datasets, evaluation results, model outputs, PHI, or run artifacts
...
Please allow a reasonable remediation period before public disclosure. We will coordinate disclosure timing with the reporter where practical.
...
Quantiles is local-first by default. Users are responsible for securing their own local workspaces, private datasets, model credentials, cloud storage, and workflow code.
